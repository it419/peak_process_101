...
const STATUS_TEXT: Record<string, string> = {
  draft: "text-dark-text-faint",
  published: "text-dark-success",
  closed: "text-dark-text-muted",
};

export function AdminJobDetailDark({ job }: { job: JobDetail }) {
  ...
  return (
    <div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {job.status === "draft" && (
            <button
              ...
              className={darkButtonVariants({ variant: "primary", size: "sm" })}
            >
              Publish
            </button>
          )}
          {job.status === "published" && (
            <>
              <button
                ...
                className={darkButtonVariants({ variant: "secondary", size: "sm" })}
              >
                Unpublish
              </button>
              <button
      ...
      <div className="mt-8">
        <AdminJobFormDark key={job.updatedAt} mode="edit" jobId={job.id} initialJob={job} />
      </div>
    </div>
  );
}
