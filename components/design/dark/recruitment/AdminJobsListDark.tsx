...
const STATUS_DOT: Record<string, string> = {
  draft: "bg-dark-text-faint",
  published: "bg-dark-success",
  closed: "bg-dark-text-muted",
};

export function AdminJobsListDark({ jobs }: { jobs: JobSummary[] }) {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="font-dark-display text-2xl font-semibold text-dark-text">Job Openings</h1>
        <Link href="/admin/jobs/new" className={darkButtonVariants({ variant: "primary" })}>
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <p className="font-dark-display text-base font-semibold text-dark-text">{job.title}</p>
                </div>
                ...
                <p className="mt-1 text-sm text-dark-text-faint">
              ...
              <div className="flex shrink-0 items-center gap-4">
                {job.status !== "draft" && (
                  <Link href={`/admin/jobs/${job.id}/applications`} className="text-sm font-medium text-dark-gold hover:underline">
                    View Applications
                  </Link>
                )}
                <Link href={`/admin/jobs/${job.id}`} className="text-sm font-medium text-dark-text hover:underline">
                  Edit
                </Link>
                {job.status === "draft" && (
                  <button
                    ...
                    className="text-sm font-medium text-dark-success hover:underline disabled:opacity-50"
                  >
                    Publish
                  </button>
                )}
